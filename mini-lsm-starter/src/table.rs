...
impl BlockMeta {
    ...
    pub fn decode_block_meta(mut buf: &[u8]) -> Result<Vec<BlockMeta>> {
        ...
        for _ in 0..num {
            ...
            let first_time_stamp = buf.get_u64();
            let first_key = KeyBytes::from_bytes_with_ts(first_key_bytes, first_time_stamp);

            let last_key_len: usize = buf.get_u16() as usize;
            let last_key_bytes = buf.copy_to_bytes(last_key_len);
            let last_time_stamp = buf.get_u64();
            let last_key = KeyBytes::from_bytes_with_ts(last_key_bytes, last_time_stamp);
